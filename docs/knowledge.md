...
### 2026-06-13: spec.md を requirements v2.0 §12 と整合 (上流→下流の同期)
spec.md (5/28 初版) は requirements v2.0 (6/13) より古く、DynamoDB キー設計が食い違っていた。(1) SK 命名: spec の暫定 `PROGRESS#`/`SM2#`/`PHONEME#` を §12 の正規命名 `RECORD#`/`ASSESS#`/`SRS#` に統合。(2) GSI: spec の `nextReviewBucket` 方式を §12 の GSI1(CEFR別語彙)/GSI2(DUE横断) に置換。(3) GSI1 の作成時期: てつてつ判断で **Phase 0 で作成** に決定 (語彙 seed 時に AP-6 レベル別出題まで動作確認するため、requirements FR-13.1/FR-14.3 と一致)。GSI2 は SRS が Phase 1 機能のため Phase 1 で追加。要件定義書(何を作るか)が上流、spec(どう作るか)が下流という原則で、上流更新時は下流を同期する運用を確認。
